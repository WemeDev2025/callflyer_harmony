...
Page({
  ...
  async checkExistingWorkCard() {
    try {
      ...
      // 如果有我的工作卡（草稿状态），显示编辑页面
      if (myResult.success && myResult.data && myResult.data.length > 0) {
        console.log('✅ 已有我的工作卡数据，显示编辑页面');
        this.setData({
          isEdit: true,
          workCardData: myResult.data[0]
        });
        this.loadWorkCardData();
      } else if (publishedResult.success && publishedResult.data && publishedResult.data.length > 0) {
        // 如果有已发布的工作卡，直接跳转到预览页面
        console.log('✅ 已有已发布工作卡数据，跳转到预览页面');
        wx.redirectTo({
